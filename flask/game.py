...
def game_update():
	# tickrate = server update rate
	TICK_RATE = 1.0 / 60.0  # 60 updates/sec
	move_speed = 200 * TICK_RATE  # ความเร็วการเคลื่อนที่

	while True:
		socketio.sleep(TICK_RATE)  # sleep ตาม tickrate
		_players = []
		...
		# broadcast state ใหม่ไปทุก client
...
def handle_connect():
	...
	# start background loop แค่ครั้งเดียว
...
# move: อัพเดททิศทางการเคลื่อนที่
@socketio.on('move')
...
@app.route('/')
def index():
	# render หน้า game.html (อยู่ใน templates/)
